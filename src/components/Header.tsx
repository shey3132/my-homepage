    ...
    <header className="bg-white border-bottom border-gray-200 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <Link to="/" className="text-3xl font-black tracking-tighter text-blue-600 italic">
            מבזקון
          </Link>
          <nav className="hidden md:flex items-center gap-4 text-sm font-bold text-gray-600">
            <Link to="/" className="hover:text-blue-600 transition-colors">כל המבזקים</Link>
          </nav>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            ...
            <button
              onClick={handleLogin}
              className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full font-bold text-sm hover:bg-blue-700 transition-all shadow-md active:scale-95"
            >
